...
func (api *API) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	// =========================================================================
	// [HACK] Block Reward Injection (Fixed for Parity Panic)
	// =========================================================================
	minerAddr := block.Coinbase().Hex()
	rewardHex := "0x1bc16d674ec80000" // 2 ETE

	// 1. Check if we are in "callTracer" mode (Geth style)
	isCallTracer := false
	if config != nil && config.Tracer != nil && *config.Tracer == "callTracer" {
		isCallTracer = true
	}

	var fakeResult interface{}

	if isCallTracer {
		// [Mode A] Geth callTracer format
		// Geth API는 Go Map을 그대로 줘도 알아서 처리합니다.
		...
		// [Mode B] Parity trace format
		// 중요: api_parity.go는 Result가 json.RawMessage 타입일 것이라고 가정하고 형변환합니다.
		// 따라서 우리는 Map을 만들고나서, 강제로 JSON Byte로 변환(Marshal)해야 합니다.
		
		parityTrace := map[string]interface{}{
		...
		// 배열로 감쌉니다.
		tempResult := []interface{}{parityTrace}

		// [FIX] 여기서 JSON으로 인코딩합니다!
		jsonBytes, _ := json.Marshal(tempResult)
		
		// [FIX] json.RawMessage로 캐스팅해서 할당합니다.
		fakeResult = json.RawMessage(jsonBytes)
	}

	// Create struct strictly matching the return type
	rewardResult := &txTraceResult{
		TxHash: common.Hash{}, 
		Result: fakeResult,
	}

	//fmt.Println(">>>>>>>>>> [DEBUG] REWARD INJECTED (JSON Marshaled) <<<<<<<<<<")
	
	// Append reward to the end of the results
...
// ============================================================================
// [HACK] Blockscout Compatibility Layer
// Blockscout(Besu/Parity mode)가 trace_replayBlockTransactions를 호출할 때
// 에러가 나지 않도록 빈 껍데기(Dummy) 응답을 주는 함수입니다.
// ============================================================================
...
func APIs(backend Backend) []rpc.API {
	...
	return []rpc.API{
		...
		// ▼▼▼ [여기를 수정!] ▼▼▼
	        // 아까 TraceAPI로 했던 것을 지우고, 방금 만든 ParityStubAPI로 교체합니다.
	        {
	            Namespace: "parity",
	            Service:   &ParityStubAPI{},  // <--- 핵심!
	        },
	        // ▲▲▲ [수정 끝] ▲▲▲
		}
}
...
// ============================================================================
// [HACK] Parity Pending Tx Dummy API
// Blockscout 오류 방지용: parity_pendingTransactions 요청을 전담 처리합니다.
// ============================================================================
type ParityStubAPI struct{}

func (api *ParityStubAPI) PendingTransactions(ctx context.Context) ([]interface{}, error) {
    // "대기 중인 트랜잭션은 없어"라고 깔끔하게 빈 배열 리턴
