...
// trace_replayBlockTransactions 요청에 대해 최소한의 더미 데이터를 응답합니다.
// ============================================================================
// ============================================================================
// [HACK] Blockscout Compatibility Layer
// trace_replayBlockTransactions 요청에 대해 더미 데이터를 응답합니다.
// ============================================================================
func (api *TraceAPI) ReplayBlockTransactions(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, traceTypes []string) ([]interface{}, error) {
	var block *types.Block
	var err error
	if hash, ok := blockNrOrHash.Hash(); ok {
		block, err = api.debugAPI.blockByHash(ctx, hash)
	...
	for i, tx := range txs {
		// [핵심] transactionHash가 있어야 Blockscout가 에러를 내지 않습니다.
		results[i] = map[string]interface{}{
			"trace":           []interface{}{},
			"stateDiff":       nil,
			"vmTrace":         nil,
			"output":          "0x",
			"transactionHash": tx.Hash(), // <--- 필수 필드
		}
	}

    // 로그 스팸 제거 (삭제됨)
	// fmt.Println(">>>>>>>>>> [DEBUG] TraceAPI: Served Dummy ReplayBlockTransactions <<<<<<<<<<")
	return results, nil
}

// ============================================================================
// [HACK] Blockscout Compatibility Layer (Pending Tx)
// parity_pendingTransactions 요청에 대해 빈 배열을 리턴하여 에러를 방지합니다.
// ============================================================================
func (api *TraceAPI) PendingTransactions(ctx context.Context) ([]interface{}, error) {
    // "대기 중인 트랜잭션 없어"라고 깔끔하게 빈 배열 리턴
